...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if r.status_code != 200:
        raise HTTPException(status_code=502, detail=f"TMDB API error ({r.status_code}): {r.text[:500]}")

    try:
        return r.json()
    except Exception:
        raise HTTPException(status_code=502, detail=f"TMDB returned invalid JSON: {r.text[:500]}")
